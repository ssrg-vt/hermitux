...
void benchmp(benchmp_f initialize, 	benchmp_f benchmark, benchmp_f cleanup,
		int enough, int parallel, int warmup, int repetitions, void* cookie);
...
//#else
		where = mmap(0, size, PROT_READ|PROT_WRITE, MAP_ANONYMOUS|MAP_SHARED, fd, 0);
//#endif
		if ((long)where == -1) {
			perror("mmap");
			exit(1);
