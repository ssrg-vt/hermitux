...
int *key_array, *key_buff1, *key_buff2, *partial_verify_vals;

#ifdef USE_BUCKETS
int * bucket_size, *bucket_ptrs;
#endif /* USE_BUCKETS */

void allocate_arrays(void) {
	key_array = malloc(SIZE_OF_BUFFERS * sizeof(int));
	key_buff1 = malloc(MAX_KEY * sizeof(int));
	key_buff2 = malloc(SIZE_OF_BUFFERS * sizeof(int));
	partial_verify_vals = malloc(TEST_ARRAY_SIZE * sizeof(int));
...
#ifdef USE_BUCKETS
	bucket_size = malloc(NUM_BUCKETS * sizeof(int));
	bucket_ptrs = malloc(NUM_BUCKETS * sizeof(int));
	if(!bucket_size || !bucket_ptrs) {
		fprintf(stderr, "cannot allocate memory\n");
		exit(-1);
	}
#endif /* USE_BUCKETS */
